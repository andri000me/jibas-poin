...
class Auth extends CI_Controller
{
	...
	public function generate_pengguna_siswa()
	{
		$this->db->query('DELETE FROM users WHERE company="siswa"');

		$siswa = $this->siswa_model->get_all_siswa();

		foreach ($siswa as $item) {

			$email = $item->nipd . '@smkplusalfarhan.sch.id';
			$identity = $email;
			$password = '12345678';

			$additional_data = [
				'siswa_id' => $item->id,
				'first_name' => strtoupper($item->nama),
				'company' => 'siswa',
			];

			$group = array('5'); // group siswa

			$this->ion_auth->register($identity, $password, $email, $additional_data, $group);
		}

		$this->session->set_flashdata('success', 'Data Pengguna siswa berhasil diatur ulang');

		redirect('users');
	}

	public function generate_pengguna_guru()
	{
		$this->db->query('DELETE FROM users WHERE company="guru"');

		$this->load->model('gtk_model');
		$this->load->model('rombel_model');

		$gtk = $this->gtk_model->get_all_gtk();

		foreach ($gtk->result() as $item) {
			...
			$additional_data = [
				'gtk_id' => $item->id,
				'first_name' => strtoupper($item->nama_gtk),
				'company' => 'guru',
			];

			if ($this->rombel_model->cek_wali($item->npa) > 0) 
			{
				$group = array('3'); // walikelas
			} 
			else 
			{
				$group = array('4'); // group guru
			}
		...
		$this->session->set_flashdata('success', 'Data Pengguna Guru berhasil diatur ulang');
	...
	public function create_user()
	{
		...
		else
		{
			...
			$this->data['gtk'] = $this->gtk_model->get_all_gtk()->result();
